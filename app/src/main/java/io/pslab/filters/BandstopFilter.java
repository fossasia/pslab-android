...
public class BandstopFilter {
    private final String TAG = "BandStopFilter";
    ...
    public BandstopFilter(int order, double[] wn) {
        ...
        List<double[]> zpk2tfArray = zpk2tf(z2, p, k2);
        a = zpk2tfArray.get(0);
        b = zpk2tfArray.get(1);
    }
    ...
    private List<Object> zpklp2bs(double[] z, Complex[] p, int k, double wo, double bw) {
        ...
        pProd = temp.getReal();
        kbs = (int) (k / pProd);
        return Arrays.asList(zbs, pbs, kbs);
    }

    private List<Object> zpkBilinear(Complex[] z, Complex[] p, int k, double fs) {
        ...
        //rearranging pz
        List<Complex> pz2 = new ArrayList<>();
        for (Complex aPz : pz) {
        ...
        return Arrays.asList(zz, pzRearranged, kz);
    }

    private List<double[]> zpk2tf(Complex[] z, Complex[] p, double k) {
        ...
        return Arrays.asList(aCoefficients, bCoefficients);
    }
