...
public class RadioLink {
    ...
    private final int NRF_READ_REGISTER = 0 << 4;
    private final int NRF_WRITE_REGISTER = 1 << 4;

    private final int MISC_COMMANDS = 4;
    private final int WS2812B_CMD = 0 << 4;

    private final NRF24L01 nrf24L01;
    ...
    public List<Character> writeI2C(int I2CAddress, int regAddress, int[] data) throws IOException {
        selectMe();
    ...
    public List<Character> readI2C(int I2CAddress, int regAddress, int numBytes) throws IOException {
        selectMe();
        return this.nrf24L01.transaction(new int[]{I2C_COMMANDS | I2C_TRANSACTION, I2CAddress, regAddress, numBytes}, 0, timeout);
    }

    public List<Character> writeBulk(int I2CAddress, int[] data) throws IOException {
        selectMe();
    ...
    public List<Character> readBulk(int I2CAddress, int regAddress, int numBytes) throws IOException {
        selectMe();
        return this.nrf24L01.transactionWithRetries(new int[]{I2C_COMMANDS | I2C_TRANSACTION, I2CAddress, regAddress, numBytes}, -1);
    }

    public List<Character> simpleRead(int I2CAddress, int numBytes) throws IOException {
        selectMe();
        return this.nrf24L01.transactionWithRetries(new int[]{I2C_COMMANDS | I2C_READ, I2CAddress, numBytes}, -1);
    }

    public List<Character> readADC(int channel) throws IOException {
        selectMe();
    ...
    public List<Integer> i2CScan() throws IOException {
        ...
        List<Character> temp = this.nrf24L01.transaction(new int[]{I2C_COMMANDS | I2C_SCAN | 0x80}, 0, 500);
    ...
    private List<Integer> decodeI2CList(int[] data) {
        int sum = 0;
    ...
    public List<Integer> pullSCLLow(int t_ms) throws IOException {
        selectMe();
        List<Character> data = this.nrf24L01.transaction(new int[]{I2C_COMMANDS | PULL_SCL_LOW, t_ms}, 0, timeout);
        if (data != null) {
    ...
    public List<Character> configI2C(int frequency) throws IOException {
        selectMe();
        int brgVal = (int) 32e6 / frequency / 4 - 1;
        return this.nrf24L01.transaction(new int[]{I2C_COMMANDS | I2C_CONFIG, brgVal}, 0, timeout);
    }

    public List<Character> writeRegister(int register, int value) throws IOException {
        selectMe();
    ...
    public int readRegister(int register) throws IOException {
        selectMe();
        List<Character> data = this.nrf24L01.transaction(new int[]{NRF_COMMANDS | NRF_READ_REGISTER, register}, 0, timeout);
        if (data != null) {
    ...
    public List<Character> WS2812B(int[][] cols) throws IOException {
        selectMe();
