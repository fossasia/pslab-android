...
public class SSD1306 {
    ...
    public SSD1306(I2C i2C) throws IOException, InterruptedException {
        ...
        params.put("load", new ArrayList<String>(Collections.singletonList("logo")));
        params.put("scroll", new ArrayList<String>(Arrays.asList("left", "right, ", "topright", "topleft", "bottomleft", "bottomright", "stop")));
    }
    ...
    public void drawChar(int x, int y, int c, int color, int bg, int size) {
        int line;
        if((x >= _width) | (y >= _height) | ((x + 5 * size - 1) < 0) | ((y + 8 * size - 1) < 0))
            return;
        for(int i = 0; i < 6; i++){
            if (i == 5)
                line = 0x0;
